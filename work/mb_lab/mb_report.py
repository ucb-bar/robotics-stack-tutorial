...
Everything is plain terminal text; `mb report`, `mb list`, `mb commands` and `mb calls` (on the
seat or the board) print it, and the notebook (notebooks/mb_lab/mb_lab.py) shows the commands.
"""
import json
import os
import sys
...
def short_diag(d) -> str:
    """ModelBlaster's diag is a whole build log joined with ' | '; its first clause says it."""
...
def board_state(r):
    """(state, board dict): 'done' when board.json was folded in, 'pending' while the
    attendee's board step is outstanding, None for a spike-only run."""
    bd = r.get("board")
...
def mbp_ops(image) -> str:
    """'MAX8 x2' -- the MBP instructions compiled into the kernel (outside the negative test)."""
    c = {}
...
def accel_line(r) -> str:
    """One line: does the new kernel run on the accelerator, and what does the accelerator buy?"""
    bs, bd = board_state(r)
    if bs != "done":
        return ""
    ...
    if bd.get("speedup_accel") and bd.get("speedup_code"):
        ...
        else:   # the rewrite only pays off WITH the instruction: without it, it loses to the reference
            s += (f" -- slower even than the reference ({bd['speedup_code']:.1f}x): the rewrite only pays "
...
# What each logged command is, for someone who has not used the tools before.
TOOLS = [
    ...
    ("generate_kernels", "ModelBlaster: the IR -> C kernels (reference or curated), host-verified"),
...
def show_commands(run: Path) -> None:
    """Every tool the run executed, exactly, labelled; paths shortened to $RUN and ~."""
    p = run / "commands.sh"
    if not p.exists():
...
def show_calls(run: Path) -> None:
    """Every LLM call of a run, in order: what was asked (the user prompt) and what came back."""
