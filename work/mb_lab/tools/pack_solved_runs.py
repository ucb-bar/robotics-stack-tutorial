...
Each run is trimmed to what the notebook reads, and each status.json loses the instance's
hostname and pid (the reason notebooks here are committed without outputs).  The archive is
deterministic -- sorted names, zero timestamps -- so packing the same runs twice gives the
same bytes.
"""
import gzip
import io
