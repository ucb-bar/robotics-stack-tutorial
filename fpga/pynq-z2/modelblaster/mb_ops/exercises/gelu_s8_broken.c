/* Exercise (docs/MB_GELU_WALKTHROUGH.md, "Spot the bug"): this is the LLM's gelu_s8 kernel with ONE change in
 * the table loop.  The spike golden still passes (see the walkthrough).  Does the whole domain
 * check agree?  Find the line, and say why a sample could miss it. */
void kernel_gelu_s8(const int8_t *input, int8_t *output, int n, float scale_in, float scale_out, int activation_min, int activation_max)
{
    ...
    for (int idx = 0; idx < 256; ++idx) {
        if (seen[idx >> 5] & (1U << (idx & 31U))) {
            ...
            float erf_val = erff(scaled_f);
            float y = 0.5f * f * (1.0f + erf_val);
            int32_t v = (int32_t)roundf(y * inv_scale_out * 1.00001f);
            if (v < activation_min) v = activation_min;
            else if (v > activation_max) v = activation_max;
            table[idx] = (int8_t)v;
        }
